...
void Send_text_telegram()
{
 ...
 %s
  ...
  if (EnableBOT)
    bot.sendMessage(BOTme, the_page, ""); // "MarkdownV2");
}
...
void print_ram()
{
  ...
  Serial.print(count_cam);
  Serial.print(" / ");
  Serial.print(count_avi);
  Serial.print(" / ");
  Serial.print(count_ftp);
  Serial.print(" / ");
  Serial.print(count_ftp2);
  Serial.print(" / ");
  ...
  if (ready)
  {
    ...
    Serial.print(" / ");
    Serial.print(uxTaskGetStackHighWaterMark(CameraTask));
    Serial.print(" / ");
    Serial.println(uxTaskGetStackHighWaterMark(FtpTask));
  }
...
static esp_err_t save_photo_dated()
{
 ...
 %s
    ...
    Serial.print("Sending Photo Telegram, bytes: ");
...
void IRAM_ATTR low_voltage_save(void *arg)
{
  ...
  Serial.print("low volt handler, core ");
  Serial.print(xPortGetCoreID());
  Serial.print(", priority = ");
  Serial.println(uxTaskPriorityGet(NULL));

  uint32_t brown_reg_temp = READ_PERI_REG(RTC_CNTL_BROWN_OUT_REG); // save WatchDog register
  Serial.print("\nBrownOut Regsiter was (in hex) ");
  Serial.println(brown_reg_temp, HEX);
...
void do_eprom_read()
{
  ...
  if (ed.eprom_good == MagicNumber)
  {
    ...
    Internet_Enabled = ed.Internet_Enabled;
    Serial.print("Internet_Enabled ");
    Serial.println(Internet_Enabled);
    DeepSleepPir = ed.DeepSleepPir;
    Serial.print("DeepSleepPir ");
    Serial.println(DeepSleepPir);
    record_on_reboot = ed.record_on_reboot;
    Serial.print("record_on_reboot ");
    Serial.println(record_on_reboot);
    PIRpin = ed.PIRpin;
    Serial.print("PIRpin ");
    Serial.println(PIRpin);
    PIRenabled = ed.PIRenabled;
    Serial.print("PIRenabled ");
    Serial.println(PIRenabled);
    framesize = ed.framesize;
    Serial.print("framesize ");
    Serial.println(framesize);
    repeat_config = ed.repeat;
    Serial.print("repeat_config ");
    Serial.println(repeat_config);
    repeat = ed.repeat;
    xspeed = ed.xspeed;
    Serial.print("xspeed ");
    ...
    capture_interval = ed.capture_interval;
    Serial.print("capture_interval ");
    Serial.println(capture_interval);
    total_frames = ed.total_frames;
    total_frames_config = ed.total_frames;
    Serial.print("total_frames_config ");
    Serial.println(total_frames_config);
    xlength = ed.xlength;
    Serial.print("xlength ");
    Serial.println(xlength);
    EnableBOT = ed.EnableBOT;
    Serial.print("EnableBOT ");
    Serial.println(EnableBOT);
  }
  else
  {
...
void make_avi()
{
  ...
  { // open the file
    ...
    if (EnableBOT == 1 && Internet_Enabled == 1)
    { //  if BOT is enabled wait to send it ... could be several seconds (5 or 10)
      // 89 config_camera();
      send_a_telegram = 1;
      Wait_for_bot = 1;
...
void loop()
{
  ...
  count_loop++;
  wakeup = millis();
  if (wakeup - last_wakeup > (13 * 60 * 1000))
  { // 13 minutes
    last_wakeup = millis();
    do_time();
...
#ifdef include_telegram
  if (send_a_telegram == 1)
  { // send the telegram after flag set, using the general heap
    send_a_telegram = 0;
    if (EnableBOT == 1 && Internet_Enabled == 1)
    { // just double-check
      save_photo_dated();
...
static esp_err_t pir_dis_handler(httpd_req_t *req)
{
  ...
  Serial.print("http pir_dis, core ");
  Serial.print(xPortGetCoreID());
  Serial.print(", priority = ");
  Serial.println(uxTaskPriorityGet(NULL));
...
static esp_err_t bot_dis_handler(httpd_req_t *req)
{
  ...
  Serial.print("http bot_dis, core ");
  Serial.print(xPortGetCoreID());
  Serial.print(", priority = ");
  Serial.println(uxTaskPriorityGet(NULL));
    ...
    if (buf_len > 1)
    {
      if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK)
      {
        ...
        if (httpd_query_key_value(buf, "quality", param, sizeof(param)) == ESP_OK)
        {
          ...
          int x = atoi(param);
          if (x >= 6 && x <= 50)
          {                  // MINIMUM QUALITY 10 to save memory
